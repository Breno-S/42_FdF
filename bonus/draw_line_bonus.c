...
/*   Updated: 2025/10/11 23:01:44 by brensant         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	draw_line_low(t_env *mlx, t_point2 p0, t_point2 p1)
{
	t_point2	delta;
	int			diff;
	int			step_y;
	...
	while (p0.x < p1.x)
	{
		env_pixel_put(mlx, p0.x, p0.y, 0xFFFFFF);
		if (diff > 0)
		{
			p0.y += step_y;
			diff += (2 * (delta.y - delta.x));
...
static void	draw_line_high(t_env *mlx, t_point2 p0, t_point2 p1)
{
	t_point2	delta;
	int			diff;
	int			step_x;
 ...
 */
void	draw_line(t_env *mlx, t_vector3 v0, t_vector3 v1)
{
	t_point2	p0;
	t_point2	p1;
